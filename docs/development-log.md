# 开发日志

## 2026-06-07 AI 原型草图工具首版

- [决策]: 在 Excalicord 基础上保留 Excalidraw 白板能力，首版产品方向改为“给 AI Agent 理解的低保真原型草图工具”。
- [进展]: 首屏改成桌面顶栏感的浮窗工作台，包含左侧绘制工具、中央 Excalidraw 画布、右侧 iPhone/Web/弹窗模板，以及“复制给 AI”和“导出”动作。
- [进展]: 移除登录、录制、摄像头、美颜、AI avatar 作为首屏主流程；旧模块源码暂时保留。
- [验证]: `npm run build` 通过；桌面与窄屏浏览器检查通过；结果记录在 `design-qa.md`。
- [踩坑]: npm 默认缓存有权限问题，本轮安装改用项目内临时缓存完成，缓存已清理。

## 2026-06-07 全屏工作台修正

- [进展]: 用户指出界面外层有问题后，去掉伪桌面顶栏和内嵌窗口感，改为真正占满可用区域的工具工作台。
- [进展]: Excalidraw 初始化后自动 fit 全部 starter 元素，避免只看到 iPhone 模板、右侧和底部出现大面积空白。
- [验证]: `npm run build` 通过；桌面浏览器复查通过，无当前 app 控制台错误。

## 2026-06-08 Cloudflare Pages 上线

- [进展]: 创建 Cloudflare Pages 项目 `ai-prototype-sketch-tool`，生产分支设置为 `main`。
- [进展]: 将本地 `dist` 直接上传到 Cloudflare Pages，正式地址为 `https://ai-prototype-sketch-tool.pages.dev/`。
- [验证]: `npm run build` 通过；Cloudflare 部署 `bcc0259e-c252-4e07-a54f-1a84e27bdcaf` 为 Production / main；正式域名与部署域名均返回 `200`。
- [决策]: 使用 Wrangler 直接上传 `dist` 到 Pages；不把 Wrangler 固定进项目依赖，避免污染当前前端依赖锁文件。

## 2026-06-08 新建画布入口

- [进展]: 顶部操作区新增「新建」按钮，点击后清空当前 Excalidraw 画布、重置插入计数，并切回选择工具。
- [验证]: `npm run build` 通过；本地页面点击「新建」后初始模板消失，画布变为空白，无相关控制台错误。

## 2026-06-09 图片导入与遮盖标注

- [进展]: 顶部操作区新增「导入图片」，支持把截图或 UI 图片放进 Excalidraw 画布，并保留移动、缩放、导出能力。
- [进展]: 左侧工具栏新增「遮盖」工具，使用粗手绘笔和 4 个常用遮盖色，方便涂掉截图中不想保留的区域。
- [验证]: `npm run build` 通过；本地桌面流程完成图片导入、遮盖色切换与涂抹；窄屏检查确认新增入口不遮挡主要画布。

## 2026-06-09 UI 组件模板

- [进展]: 右侧面板新增「组件」区，提供搜索框、列表卡片、顶部导航、底部导航、表单项、主按钮 6 个低保真 UI 组件。
- [进展]: 组件插入后保持为可编辑 Excalidraw 元素，方便继续拖动、改文字、调整尺寸。
- [验证]: `npm run build` 通过；本地桌面宽度点击「搜索框」「列表卡片」可插入画布，控制台无相关错误。

## 2026-06-09 草图交互预览

- [进展]: 顶部新增「生成预览」，可把当前画布导出成预览图，并基于画布中的短文本/小形状生成可点击状态按钮。
- [进展]: 预览层支持状态切换和打开/关闭弹窗，用于把静态草图快速变成可点击的低保真原型。
- [验证]: `npm run build` 通过；本地桌面与窄屏均可打开交互预览、切换状态、打开/关闭弹窗，控制台无相关错误。

## 2026-06-09 回到 AI 交接包

- [决策]: 撤掉顶部「生成预览」，避免产品偏向“看起来会生成交互”的演示，而不是解决给 AI Agent 交接原型的问题。
- [进展]: 顶部主动作改为「复制交接包」，复制当前 PNG 的同时生成结构化文本，包含画布概况、草图文字、交互线索和实现要求。
- [验证]: `npm run build` 通过；本地桌面和窄屏确认「生成预览」已消失，「复制交接包」存在；点击后可复制结构化交接文本，控制台无相关错误。
